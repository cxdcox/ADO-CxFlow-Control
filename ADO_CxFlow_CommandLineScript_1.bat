...
set CX_PROJECT=ADO-12-CxFlow-MyFirstProject
set CX_APP=ADO-12
...
echo "Invoking: java -jar cx-flow-1.6.22.jar --scan --f="$(Build.Repository.LocalPath)" --spring.config.location="application-azure_v9.4.0.yml" --cx-team="%CX_TEAM%" --cx-project="%CX_PROJECT%" --app="%CX_APP%" --namespace="cx-drc" --repo-name="$(Build.Repository.Name)" --branch="$(Build.SourceBranchName)" "
java -jar cx-flow-1.6.22.jar --scan --f="$(Build.Repository.LocalPath)" --spring.config.location="application-azure_v9.4.0.yml" --cx-team="%CX_TEAM%" --cx-project="%CX_PROJECT%" --app="%CX_APP%" --namespace="cx-drc" --repo-name="$(Build.Repository.Name)" --branch="$(Build.SourceBranchName)"
